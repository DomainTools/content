...
def http_request(method, params=None):
    ...
    try:
        if method == "iris-investigate":
            ...
            nod_feeds = []
            for row in rows:
                feed_json_result = json.loads(row)
                nod_feeds.append(
                    {
                        "timestamp": feed_json_result.get("timestamp"),
                        "domain": feed_json_result.get("domain"),
                    }
                )
            return nod_feeds
        else:
...
def fetch_indicators():
    nod_feeds = http_request("nod-feeds")

    feed_indicators = []
    for feed in nod_feeds:
        domain = feed.get("domain") or None
        ...
        feed_indicators.append(
            {
                "type": dt_indicator["type"],
                "value": dt_indicator["value"],
                "occured": datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%SZ"),
                "rawJSON": dt_indicator,
                "score": None,
                "fields": {
                    "source": "DomainTools Iris NOD Feeds",
                    "sourcebrands": "DomainTools Iris",
                    "firstseen": convert_and_format_date(timestamp),
                    "sourceinstances": "DomainTools Iris",
                },
            }
        )

    total_feeds = len(feed_indicators)
    readable_output = f"Total of newly observed domains: {total_feeds}"

    feed_indicators = feed_indicators[:50]

    # we submit the indicators in batches
    for bi in batch(feed_indicators, batch_size=BATCH_SIZE):
        demisto.createIndicators(bi)

    results = CommandResults(readable_output=readable_output, ignore_auto_extract=True)
...
def main():
    ...
    try:
        if demisto.command() == "test-module":
            ...
            fetch_domains()
        elif demisto.command() == "domaintools-nod-feeds":
            fetch_indicators()
    except Exception as e:
        return_error(
